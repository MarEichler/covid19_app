      ...
      mainPanel(
        tabsetPanel(type = "tabs", id = "tabsetpanel_map"  , selected = 11, #SELECT TAB 
          tabPanel(
            ...
            value = 11, 
            br(), 
            imageOutput(ns("mapimg")  , width = "98%", height = "98%") %>% withSpinner()
            ), #end tabPanel<Hex Map
          tabPanel(
            title = "Table", 
            value = 12, 
            br(), 
...
map_Server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    var <- mod_filters_Server("var", nDATES = 1, GEO = FALSE)
    ...
    # create image 
    output$mapimg   <- renderImage({
      plotit <- p()
      out_width   <- session$clientData[[glue("output_{id  }-mapimg_width" )]]
